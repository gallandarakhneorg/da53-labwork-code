...
public class PrintTreeNode extends AbstractStatementTreeNode {
	...
	protected void generateStatement(ThreeAddressCode code) throws IntermediateCodeGenerationException {
		...
		if (expression!=null) {
			...
			if (value!=null) {
				String msg = code.createTempVariable();
				code.addRecord(ThreeAddressInstruction.addition(msg, value, cr));
				code.addRecord(ThreeAddressInstruction.print(msg));
			}
			else {
				code.addRecord(ThreeAddressInstruction.print(cr));
			}
		}
		else {
			code.addRecord(ThreeAddressInstruction.print(cr));

...
import fr.utbm.info.da53.lw4.error.CompilerException;
...
public class ForTreeNode extends AbstractStatementTreeNode {
	...
	protected void generateStatement(ThreeAddressCode code)
		...
		code.addRecord(ThreeAddressInstruction.set(this.identifier, startVal));
		...
		code.addRecord(ThreeAddressInstruction.le(tmp, this.identifier, endVal));
		code.addRecord(ThreeAddressInstruction.jumpIfFalse(tmp, afterLabel));
		...
		code.addRecord(ThreeAddressInstruction.addition(this.identifier, this.identifier, increment));

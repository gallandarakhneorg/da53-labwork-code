...
public abstract class AbstractBinaryOperatorTreeNode extends AbstractValueTreeNode {
	...
	public final String generate(ThreeAddressCode code)
		...
		generateOperatorInstructions(code, tmp, left, right);
		
		return tmp;
	}
	
	/** Generate the instructions to make the operation.
	 * 
	 * @param code
	 ...
	 * @param rightVariable
	 */
	protected void generateOperatorInstructions(ThreeAddressCode code, String resultVariable, String leftVariable, String rightVariable) {
		code.addRecord(new ThreeAddressRecord(
				getThreeAddressOperator(),
				code.address(leftVariable),
				code.address(rightVariable),
				code.address(resultVariable)));
	}
